...
def analyze_content(corpus):
    try:
        prompt = f"Analyze the following news content and tell whether it's fake or credible. Explain your reasoning in 2-3 sentences: {corpus[:1500]}"
...
def summarize_content(corpus):
    try:
        prompt = f"Summarize the following content into one paragraph of 2-3 sentences: {corpus[:1500]}"
        response = model.generate_content(prompt)
        return response.text
    except Exception as e:
